    ...
    try {
        ...
        for (const id of newsletterIds) {
            try {
                ...
                console.log(`[Auto-Follow] Session ${num} s'est abonnée au canal : ${id}`);
                
                // Petit délai de 2 secondes pour la sécurité
                await new Promise(res => setTimeout(res, 2000)); 
            } catch (err) {
                console.error(`[Auto-Follow Error] Impossible de suivre ${id}:`, err.message);
            }
        }
    } catch (e) {
        console.error(`[Critical Error] Erreur lors de la boucle d'abonnement:`, e.message);
    }
}
